...
Decawave::Decawave(shared_ptr<SharedSPI> sharedSPI, PinName nCs, PinName intPin)
    ...
    if (_isInit) {
        ...
        setSPIFrequency(
            8000000);  // 8 MHz (doesn't work for some reason if higher)

        dwt_setrxaftertxdelay(TX_TO_RX_DELAY_UUS);
        dwt_setcallbacks(NULL,
                         static_cast<dwt_cb_t>(&Decawave::getData_success),
                         NULL, static_cast<dwt_cb_t>(&Decawave::getData_fail));
...
void Decawave::reset() { dwt_softreset(); }
...
bool Decawave::isConnected() const { return _isInit; }
...
void Decawave::getData_success(const dwt_cb_data_t* cb_data) {
    if (FRAME_LEN_MAX <= cb_data->datalength) {
        LOG(WARN,
            "Frame recieved too large:\r\n"
            "   Recieved: %u Max: %u",
            cb_data->datalength, FRAME_LEN_MAX);
...
void Decawave::logSPI(int num) {
    LOG(INIT, "spi %d %p %p", num, (int)&_spi,
        *reinterpret_cast<char*>((void*)&_spi));
}

void Decawave::setLED(bool ledOn) { dwt_setleds(ledOn); }
